...
const { BadRequestError } = require('@gw/wind-core-http'),
    ...
    debug = require('debug')('apollo:asset-repo'),
...
module.exports = class ApplicationRepoService {
    ...
    async getPackageListByType(type) {
        const assetTagsColl = this.repo.getCollection('asset-tags'),
            ...
            targets = await assetTagsColl.find({
                type
            });
    ...
    async sceduleUpdateAllVersions() {
        const assetsColl = this.repo.getCollection('assets'),
            // 更新所有资源的版本情况
            allAssets = await assetsColl.find();
    ...
    async unRegisterAsset({ name }) {
        ...
        const assetTagsColl = this.repo.getCollection('asset-tags'),
            ...
            // 删除现有的所有tag数据
            tagsRemoved = await assetTagsColl.remove({
                name
            }),

            assetsColl = this.repo.getCollection('assets'),
            // 检查是否是新增的包
            assetRemoved = await assetsColl.remove({
                name
            });
    ...
    async pullAsset({ name, type }) {
        ...
        const tags = packageInfo['dist-tags'],
            assetTagsColl = this.repo.getCollection('asset-tags');
        ...
        const assetsColl = this.repo.getCollection('assets'),
            // 检查是否是新增的包
            existed = await assetsColl.findOne({
                name
            });
