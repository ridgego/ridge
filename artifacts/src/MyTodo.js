export default {
  state: {
    ...
    todoList: [],
    filteredList: (ctx) => {
      return ctx.todoList.filter(todo => {
        if (ctx.filter === 'all') {
          return true
        } else if (ctx.filter === 'finished') {
  ...
  config: {
    ...
    reducers: {
      addTodo: '增加待办',
      toggleTodoItemStatus: '切换任务完成状态',
      setAll: '设置显示全部待办',
      removeTodoItem: '删除待办任务',
      setFilterTodo: '设置显示未完成待办',
      setFinishedTodo: '设置显示已完成待办'
    }
  }
}
