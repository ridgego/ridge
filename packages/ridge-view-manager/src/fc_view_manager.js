...
import { FCLoader } from 'moving-component-render';
...
import { I18nManager } from 'moving-runtime-i18n';
...
class FCViewManager {
     ...
     */
    constructor({ baseUrl, app, apolloApp, project }) {
        /** @property 组件加载器 */
        this.loader = new FCLoader(baseUrl);
        if (project) {
            this.loader.setProjectId(project);
        }
        if (app) {
        ...
        this.apolloApp.viewManager = this;
        ...
        this.viewDecorators = {
            ...
            'data-status': new DataStatusDecorator(this),
        ...
        this.i18nManager = new I18nManager(apolloApp, app);
    }

    async init() {
        await this.debugManager.initDebug();
        for (const decorator of Object.values(this.pageDecorators)) {
            await decorator.init(this);
        }
        await this.i18nManager.initCurrentLocale();
    }

    getDebugConfig() {
        return this.debugManager.debugConfig;
    }
